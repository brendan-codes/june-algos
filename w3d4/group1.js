...
class SLL {
    ...
    partition(val) {
        ...
        while(runner) {
            var nextVal = runner.next;
            if(runner.data < val) {
                this.delete(runner.data);
                this.addToFront(runner);
            }
            if(runner.data > val) {
                this.delete(runner.data);
                this.addToBack(runner);
            }
            runner = nextVal;
        }
    }
//Traverse the list
//If the value we encounter is less than the value given then delete and add it to the front
//If the value is greater than the given value then we delete and add to the back
//If the value is equal to the value then we do nothing
...
var sll = new SLL();
sll.addToFront(new Node(8));
sll.addToFront(new Node(2));
sll.addToFront(new Node(1));
sll.addToFront(new Node(4));
sll.addToFront(new Node(3));
sll.addToFront(new Node(7));
sll.addToFront(new Node(5));
sll.partition(4);
console.log("----------------");